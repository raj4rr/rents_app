			...
			<div class="row">
				<table id="myTable" class="table table-bordered table-striped">
					<thead>
						...
						<th>sub_name</th>
						<th>test_desc</th>
						<th>test_meta_desc</th>
						<th>test_title</th>					
						<th>test_keyworlds</th>
							<th>Actions</th>
					</thead>
					<tbody>
						<?php
							...
							$sub_id = $_GET['sub_id'];
							$sql = "SELECT test.test_id as test_id,test.test_name as test_name ,test.test_desc as test_desc,test.test_meta_desc as test_meta_desc,test.test_title as test_title,test.test_keyworlds as test_keyworlds,test.status as status,test.total_que as total_que, s.sub_name as sub_name,s.sub_id as sub_id FROM mst_test test inner join mst_subject s on test.sub_id= s.sub_id where test.status=2 and test.sub_id=".$sub_id;
							...
							while($row = $query->fetch_assoc()){
								...
								"<tr>
									...
									<td>".$row['sub_name']."</td>
									<td>".$row['test_desc']."</td>
									<td>".$row['test_meta_desc']."</td>
									<td>".$row['test_title']."</td>
									<td>".$row['test_keyworlds']."</td>
