...
$rsprev = mysql_query("SELECT id FROM abes.`$tablename` WHERE  id<'$currentpageID' and language='$option' AND hide='NO' ORDER BY id DESC limit 1") or die("Pgination1::-State query error!". mysql_error());
...
$rsnext = mysql_query("SELECT  id  FROM abes.`$tablename` WHERE  id>'$currentpageID' and language='$option' AND hide='NO' ORDER BY id asc limit 1 ") or die("Pgination2::-State query error!");
if(!mysql_numrows($rsnext)>0)
    ...
    $rs = mysql_query("SELECT  *  FROM abes.`$tablename` WHERE  id='$currentpageID' and language='$option' AND hide='NO'") or die("State query error!");
