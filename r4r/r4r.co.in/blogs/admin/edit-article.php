...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
		...
		$article->title = trim($_POST['title']);
			...
			$article->meta_desc = trim($_POST['meta_desc']);
	$article->content = $_POST['content'];
	$article->published_at = $_POST['published_at'];
	$article->main_menu = $_POST['main_menu'];
	$article->modified_user = $_SESSION['user_id'];
	$article->modified_by = date("Y-m-d");

	$category_ids = $_POST['category'];

		if($article->update($conn)){
				$article->setCategories($conn, $category_ids);
				Url::redirect("/blogs/admin/article.php?id={$article->id}");	// relative link
			}
		}
?>
