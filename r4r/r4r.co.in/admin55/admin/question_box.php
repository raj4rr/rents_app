...
<form action="r4r_interview_insert_q.php" method="post">
	<table style="width:100%;">
		...
		<tr><td colspan="3"><p>Answer:</p>
		<script type="text/javascript" src="../admin_js/jquery-te-1.4.0.min.js" charset="utf-8"></script>
		<textarea name="answer" id="answer" class="jqte-test" placeholder="Answer.."></textarea>
		<script>$('.jqte-test').jqte();</script>
		<?php $sql_answer = mysql_query("select * from answer where question_id='$_GET['question_id']'",$r4r_con);
		while ($row_answer = mysql_fetch_array($sql_answer))
               { 
	?>

		<tr><td colspan="3"><p>Answer:</p>

		<?php echo (stripslashes($row_answer['answer']));?>
		<td class="alternate1" style="text-align:center;"><a style=" cursor:pointer;" onClick="question_box_ans('<?php echo $row_answer['question_id']; ?>','<?php echo $row_answer['answer_id']; ?>')"><img src="../images/edit.png" style="padding:5px;"></a></td>
		</td></tr>
		<?php }?>
