...
$MAX_REC_PER_PAGE=20;
  ...
  $rs = mysql_query("SELECT COUNT(*) FROM comment WHERE hide='NO' and url='$url'") or die("Count query error!");
...
<?php }
echo '<br/>';
$result = mysql_query("SELECT * FROM comment WHERE hide='NO' and url='$url' order by id desc LIMIT $start, $max ");
...
<b>URL :</b><a href='<?= htmlspecialchars($url);  ?>'><?= htmlspecialchars($url);  ?></a><br>
<hr>
