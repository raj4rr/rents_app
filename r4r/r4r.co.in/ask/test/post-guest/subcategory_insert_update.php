...
{
	//$category=mysql_fetch_assoc(mysql_query("select category_id from subcategory where publish='Y' "));	
	$category_id=17;
}
?>
...
<div class="tableContent" style="font-size:13px;">
    <table width="100%">
<?php  
	$sql_show_category = mysql_query("select * from subcategory where category_id=$category_id");
...
<script>
	...
	<form name="form<?php echo $row_show_category['subcategory_id'];?>" action="update_subcategory.php" method="post">
    	<tr>
			...
			<td class="alternate2" style="width:10%; text-align:center;">
			<?php
			$no_of_topic= mysql_num_rows(mysql_query("select * from topic where subcategory_id=$row_show_category[subcategory_id]"));
			if($no_of_topic==0)
			{?>
