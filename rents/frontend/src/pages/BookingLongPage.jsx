...
export default function BookingLongPage() {
  ...
  const gallery = useMemo(() => {
    ...
    // Also pull in room and apartment images
  ...
  // Reset hero when listing changes
  useEffect(() => {
  ...
  // When selected listing changes, ensure months default to minimum
  useEffect(() => {
    const minM = selectedListing?.minStayMonths || 1;
    setLongTermMonths(minM);
  }, [selectedListing]);

  // When check-in or months change, auto-fill check-out and compute minimum check-out date
  useEffect(() => {
    const minM = selectedListing?.minStayMonths || 1;
    const months = Math.max(Number(longTermMonths) || 1, minM);
    if (form.checkIn) {
      const minDate = addMonthsToDate(form.checkIn, minM);
      setCheckOutMin(minDate);
      const co = addMonthsToDate(form.checkIn, months);
      setForm((prev) => ({ ...prev, checkOut: co }));
    } else {
      setCheckOutMin('');
      setForm((prev) => ({ ...prev, checkOut: '' }));
    }
  }, [form.checkIn, longTermMonths, selectedListing]);
  ...
  const unavailableRanges = useMemo(() => {
    ...
    availability.forEach(({ date, available }) => {
      if (!available) {
        ...
        ranges.push({ from: rangeStart, to: previousDate });
        rangeStart = null;
        previousDate = null;
      }
    });
    if (rangeStart) ranges.push({ from: rangeStart, to: previousDate });
    return ranges;
  }, [availability]);
  ...
  function rangeIsAvailable(start, end) {
    ...
    while (cursor < endDate) {
      ...
      // Only block if the date is explicitly marked unavailable;
      // dates not in the map (beyond fetched window) are treated as available
  ...
  const isValidLongTermStart = (date) => {
    if (availabilityMap[date] === false) return false;
    const day = Number(date.slice(8, 10));
    if (![1, 15].includes(day)) return false;
    const minM = selectedListing?.minStayMonths || 1;
    const months = Math.max(Number(longTermMonths) || 1, minM);
    const candidateEnd = addMonthsToDate(date, months);
    return rangeIsAvailable(date, candidateEnd);
  };

  const findNextValidStart = (dayOfMonth) => {
    ...
    for (let i = 0; i < 365; i += 1) {
      ...
      if (isValidLongTermStart(ds)) return ds;
    }
    return null;
  };

  const validCheckInOptions = useMemo(() => {
    if (!selectedListing) return [];
    const options = [];
    const start = new Date();
    for (let i = 0; i < 365 && options.length < 4; i += 1) {
      const d = new Date(start);
      d.setDate(d.getDate() + i);
      const day = d.getDate();
      if (![1, 15].includes(day)) continue;
      const ds = d.toISOString().slice(0, 10);
      if (isValidLongTermStart(ds)) options.push(ds);
    }
    return options;
  }, [availabilityMap, selectedListing, longTermMonths]);
  ...
  const chooseNextDay = (dayOfMonth) => {
    const ds = findNextValidStart(dayOfMonth);
    if (!ds) {
  ...
  const getQuote = async () => {
    // Validate long-term rules
    if (!form.checkIn || !form.checkOut) {
      setMessage('Select both check-in and check-out dates');
      return;
    }
    const day = Number(form.checkIn.slice(8, 10));
    if (![1, 15].includes(day)) {
      setMessage('Long-term check-in must be on the 1st or 15th of a month.');
      return;
    }
    if (!rangeIsAvailable(form.checkIn, form.checkOut)) {
      setMessage('Selected range includes booked dates. Choose a different start date or months.');
      return;
    }
  ...
  const generateContract = async () => {
    ...
    try {
      const { data } = await client.post('/contracts/generate', {
        ...
        leaseType: selectedListing?.stayType || 'LONG_TERM',
  ...
  return (
    <section>
      <h2>Create Booking (Long-term)</h2>
      ...
      <div className="card form-card" style={{ marginBottom: 14 }}>
        <select name="listingId" value={form.listingId} onChange={(e) => setForm({ listingId: e.target.value, checkIn: '', checkOut: '' })} required>
          ...
          {listings.filter(l => l.stayType === 'LONG_TERM').map((l) => <option key={l.id} value={l.id}>{l.title} (#{l.id})</option>)}
        </select>
      </div>
          ...
          <div className="listing-detail-body">
            <div className="listing-price-highlight">
              ...
              <span className="price-label">
                / month · {selectedListing.rentType === 'WARM' ? 'Warmmiete' : 'Kaltmiete'}
                {selectedListing.minStayMonths > 1 && ` · min ${selectedListing.minStayMonths} months`}
              </span>
            </div>
            ...
            <div className="listing-detail-grid" style={{ marginTop: 14 }}>
              ...
              <p><strong>Minimum Stay:</strong> {selectedListing.minStayMonths} month(s)</p>
        ...
        {form.listingId && (
          <>
            <div style={{ marginTop: 10 }}>
              <label style={{ marginRight: 10 }}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (availabilityMap[val] === false) {
                      setMessage('Selected check-in date is not available');
                      setForm((p) => ({ ...p, checkIn: '' }));
                      return;
                    }
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const val = e.target.value;
                    if (checkOutMin && val < checkOutMin) {
                      setMessage(`Check-out must be on or after ${checkOutMin}`);
                      setForm((p) => ({ ...p, checkOut: '' }));
                      return;
                    }
                    if (!rangeIsAvailable(form.checkIn, val)) {
                      setMessage('Selected range includes booked dates. Choose different months or start date.');
                      setForm((p) => ({ ...p, checkOut: '' }));
                      return;
                    }
                    setMessage('');
                    setForm((p) => ({ ...p, checkOut: val }));
                  }}
                />
              </label>
            </div>
            ...
            <div style={{ marginTop: 10 }} className="long-term-controls">
              <label style={{ marginRight: 10 }}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const min = selectedListing?.minStayMonths || 1;
              ...
              {checkOutMin && <p className="muted">Earliest checkout when starting on selected check-in: {checkOutMin}</p>}
              {unavailableRanges.length > 0 && (
                <div style={{ marginTop: 8 }}>
                  <p className="muted"><strong>Unavailable booking ranges:</strong></p>
                  <ul>
                    {unavailableRanges.map((range) => (
                      <li key={`${range.from}-${range.to}`}>{range.from} to {range.to}</li>
                    ))}
                  </ul>
                </div>
              )}
              {validCheckInOptions.length > 0 && (
                <div style={{ marginTop: 8 }}>
                  <p className="muted"><strong>Valid long-term check-in options:</strong> {validCheckInOptions.join(', ')}</p>
                </div>
              )}
            </div>
          </>
